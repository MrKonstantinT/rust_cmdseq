...
pub fn get_command_list(command: &str) -> (Vec<CmdSeq>, usize) {
    ...
    let mut num = 0;
    while num <= count_white_space(command) {
        // Using 'while' loop as 'step_by()' has issues atm.
        let t_b_n: usize = collect_between_white(command, num, num + 1)
            .parse()
            .expect("Usage: cmdseq [-d <count dir>] <count1> <cmd1> [... <countn> <cmdn>]");
        num_operations += t_b_n;
        command_list.push(CmdSeq::new(t_b_n,
                                      collect_between_white(command, num + 1, num + 2)
                                          .replace("\"", "")));
        num = num + 2;
    }
